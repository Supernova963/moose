...
OrderParameterFunctionMaterialTempl<is_ad>::OrderParameterFunctionMaterialTempl(
  ...
  : DerivativeMaterialInterface<Material>(parameters),
    ...
    _prop_df(this->template declarePropertyDerivative<Real, is_ad>(
        _function_name, _eta_name)),
    _prop_d2f(this->template declarePropertyDerivative<Real, is_ad>(
        _function_name, _eta_name, _eta_name))
{
}
