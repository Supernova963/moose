...
template <bool is_ad>
BarrierFunctionMaterialTempl<is_ad>::BarrierFunctionMaterialTempl(const InputParameters & parameters)
...
BarrierFunctionMaterialTempl<is_ad>::computeQpProperties()
{
  ...
  if (_well_only && MetaPhysicL::raw_value(n) >= 0.0 &&
      MetaPhysicL::raw_value(n) <= 1.0)
  {
