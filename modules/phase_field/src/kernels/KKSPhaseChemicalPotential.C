...
KKSPhaseChemicalPotentialTempl<is_ad>::KKSPhaseChemicalPotentialTempl(
  ...
  : DerivativeMaterialInterface<JvarMapKernelInterface<GenericKernel<is_ad>>>(parameters),
    ...
    _dfadca(this->template getMaterialPropertyDerivativeByName<Real, is_ad>(
        _fa_name, _var.name())),
    _dfbdcb(
        this->template getMaterialPropertyDerivativeByName<Real, is_ad>(_fb_name, _cb_name)),
    _ka(this->template getParam<Real>("ka")),
    _kb(this->template getParam<Real>("kb"))
{
#ifdef DEBUG
  _console << "KKSPhaseChemicalPotential(" << this->name() << ") " << _var.name() << ' '
           << _cb_name << '\n';
#endif
}
...
KKSPhaseChemicalPotential::KKSPhaseChemicalPotential(const InputParameters & parameters)
  : KKSPhaseChemicalPotentialTempl<false>(parameters),
    _d2fadca2(getMaterialPropertyDerivativeByName<Real>(
        _fa_name, _var.name(), _var.name())),
    _d2fbdcbca(
        getMaterialPropertyDerivativeByName<Real>(_fb_name, _cb_name, _var.name())),
    ...
    _d2fbdcbdarg(_n_args)
{
  for (std::size_t i = 0; i < _n_args; ++i)
  {
    ...
    _d2fadcadarg[i] =
        &getMaterialPropertyDerivativeByName<Real>(_fa_name, _var.name(), arg_name);
    _d2fbdcbdarg[i] =
        &getMaterialPropertyDerivativeByName<Real>(_fb_name, _cb_name, arg_name);
  }
}
...
KKSPhaseChemicalPotential::computeQpJacobian()
{
  return _test[_i][_qp] * _phi[_j][_qp] *
         (_d2fadca2[_qp] / _ka - _d2fbdcbca[_qp] / _kb);
}
